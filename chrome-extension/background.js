...
class SafeGuardService {
  constructor() {
    ...
    this.googleSafeBrowsingApiKey = "Your-Api-Key" // Add your Google Safe Browsing API key
    this.virusTotalApiKey = "Your-Api-Key" // Add your VirusTotal API key
  ...
  // Backend API (FastAPI) - change if your backend runs on a different host/port
  this.backendUrl = "http://localhost:8000" // e.g. http://localhost:8000
  ...
  async checkMaliciousURLDualAPI(url) {
    try {
      console.log("Checking URL with backend+dual API protection:", url)

      // First, try the project's backend which integrates ML + DB + threat feeds
      try {
        const backendResult = await this.checkBackendPredict(url)
        if (backendResult && (backendResult.prediction === 'malicious' || backendResult.prediction === 'blocked')) {
          console.log('Backend detected malicious URL:', backendResult)
        ...
        console.warn('Backend /predict-url unavailable or failed, falling back to external APIs:', err)
      }

      // Run both external API checks in parallel for faster response (fallback)
      console.log('Falling back to Google Safe Browsing and VirusTotal checks')
  ...
  async checkBackendPredict(url) {
    // Posts to backend /predict-url endpoint. Adjust body to match backend URLRequest model.
    const endpoint = `${this.backendUrl.replace(/\/$/, '')}/predict-url`
    const settings = await this.getSettings()

    const body = {
      url: url,
      child_mode: !!settings.childMode,
    ...
    // Expecting response fields: prediction, confidence, reason
    return {
      prediction: data.prediction || data.prediction || 'unknown',
  ...
  async reportToBackend(url, kind) {
    // kind: 'malicious' or 'safe'
    const endpoint = `${this.backendUrl.replace(/\/$/, '')}/${kind === 'malicious' ? 'report-malicious' : 'report-valid'}`

    const body = {
      url: url,
      report_type: kind === 'malicious' ? 'false_negative' : 'false_positive',
      source: 'extension',
      timestamp: new Date().toISOString(),
      block_reason: kind === 'malicious' ? 'user_report' : undefined,
    }
    ...
    if (!resp.ok) {
      const text = await resp.text()
      throw new Error(`Backend report failed ${resp.status}: ${text}`)
    }
  ...
  async handleMessage(request, sender, sendResponse) {
    switch (request.action) {
      ...
      case 'reportCurrentAsMalicious':
        try {
          // Expect request.url to be provided
          if (!request.url) throw new Error('No URL')
          const resp = await this.reportToBackend(request.url, 'malicious')
          sendResponse({ success: true, resp })
        } catch (err) {
          console.error('Error reporting malicious from popup:', err)
          sendResponse({ success: false, error: String(err) })
        }
        break

      case 'reportCurrentAsSafe':
        try {
          if (!request.url) throw new Error('No URL')
          const resp = await this.reportToBackend(request.url, 'safe')
          sendResponse({ success: true, resp })
        } catch (err) {
          console.error('Error reporting safe from popup:', err)
          sendResponse({ success: false, error: String(err) })
        }
        break
    }
